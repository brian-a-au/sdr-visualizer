...
The performance budgets are part of the stable
[`PRODUCT_CONTRACT.md`](PRODUCT_CONTRACT.md#stable-public-surfaces). The
visualizer must hit these targets:
...
The tier guarantees cover reports with at most 8,000 reference edges. Reports
above that edge envelope remain valid, but graph rendering requires explicit
opt-in and the size/latency budgets do not apply. The release corpus gate
reports an edge-envelope violation so maintainers can calibrate the bound
against current production data before publishing.
...
- **`scripts/perf_check.py`** — build time + HTML size, all four tiers. Generated 100- and 500-component fixtures (`generate_large_fixture.py --scale 0.083` / `--scale 0.417`) run against the small-tier budgets, the bundled CJA (1,200 components) and AA (~900) fixtures against the 1,000-component budgets, and a generated ~2,000-component XL fixture (`--scale 1.67`) against the 2,000-component budgets. Build time is the median of 3 runs. A comparative case builds the large CJA fixture against a deterministically mutated copy (`scripts/mutate_fixture.py`) through the full `--compare-to` path; budget: 1.5x the 1,000-component build budget and the tier size budget + 0.5 MB. A trend case builds a 6-snapshot series (the large CJA fixture progressively mutated in memory) through the full `--trend` path; budget: 1 s per snapshot of build time and the tier size budget + 0.5 MB.
- **`scripts/perf_browser_check.py`** — initial render time + cold and warm filter/search latency + graph-init main-thread block, measured in headless Chromium via Playwright (`uv sync --group browser`, `uv run playwright install chromium`). The embedded Promise-returning `window.__sdrPerf.timeFilter()` hook applies each filter between animation frames, forces catalog layout, and resolves on the following frame; alternating queries ensure every sample changes the result set. Required generated 100- and 500-component fixtures enforce their < 200/< 50 ms and < 500/< 100 ms render/filter budgets without graph opt-in. The bundled CJA (1,200-component) and AA (900-component) fixtures enforce the 1,000-component budgets (< 1 s, < 150 ms), and the required generated 2,004-component XL fixture enforces the 2,000-component budgets (< 2 s, < 300 ms). Exact component counts and graph-path expectations are checked before results are accepted; any missing declared fixture fails the gate. It also enforces a 700 ms graph-init cap against the tier fixtures and a deterministic ~1,000-node/~8,000-edge graph at the supported edge boundary. A disjoint-ID ~2,000-component comparison proves Changes is empty before entry and materializes in batches of at most 250. A 60-snapshot high-churn trend proves no interval summaries or ID chips render eagerly, at most 59 summaries appear on entry, and the first expanded interval remains ID-batch bounded.
...
- **Bounded DOM.** Catalog search is debounced (120 ms) and at most 1,000 rows render initially. Changes and Trend initialize only when their view is first entered. Changes filters the complete in-memory arrays before rendering fixed 250-row batches, with a hard 1,000-result ceiling that asks the user to refine instead of offering an unbounded "show all." Trend emits at most 59 interval summaries and creates ID chips only when an interval is expanded, in fixed 100-ID batches per change kind.
