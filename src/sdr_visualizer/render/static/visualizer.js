...
(function () {
  ...
  function restoreFromHash() {
    ...
    if (typesParam !== null) {
      ...
      typesParam.split(",").forEach(function (t) {
        if (KNOWN_TYPES.has(t)) { wanted.add(t); validCount++; }
      });
    ...
    var VALID_SORT_KEYS = new Set(["name", "type", "in_degree", "modified_at"]);
    var sortParam = params.get("sort");
    if (sortParam && VALID_SORT_KEYS.has(sortParam)) {
      sortKey = sortParam;
  ...
  function rowHtml(entry) {
    ...
    if (!rowHtmlCache.has(entry)) rowHtmlCache.set(entry, buildRowHtml(entry));
    return rowHtmlCache.get(entry);
  }
  ...
  function buildRowHtml(entry) {
    ...
    return (
      '<tr data-id="' + escapeHtml(entry.id) + '">' +
        ...
        '<td class="col-refs num' + (refs === 0 ? " zero" : "") + '">' + escapeHtml(refs) + "</td>" +
  ...
  function detailHtml(entry) {
    ...
    if (entry.type === "calculated_metric") {
      ...
      if (typeof entry.complexity_score === "number") {
        calcMeta += "<dt>Complexity</dt><dd>" + escapeHtml(entry.complexity_score) + "</dd>";
      }
      calcMeta += "</dl>";
      ...
      segMeta += "<dt>Nesting depth</dt><dd>" + escapeHtml(entry.nesting_depth || 0) + "</dd>";
  ...
  function workspaceUsageHtml(entry) {
    ...
    var componentLimits = Array.from(new Set(row ? (row.component_limitations || row.limitations) : []));
    var componentLimitSet = new Set(componentLimits);
    var collectionLimits = Array.from(new Set(
      (collection.limitations || []).concat(collection.retrieval ? collection.retrieval.limitations : [])
    )).filter(function (limit) { return !componentLimitSet.has(limit); });
    if (componentLimits.length) html += '<h4>Component limits</h4><ul>' + componentLimits.map(function (limit) {
      return '<li>' + escapeHtml(limit) + '</li>';
    }).join("") + '</ul>';
    if (collectionLimits.length) {
      html += '<details class="workspace-collection-limits"><summary>Collection-wide limits (' + collectionLimits.length + ')</summary><ul>' +
        collectionLimits.map(function (limit) { return '<li>' + escapeHtml(limit) + '</li>'; }).join("") +
        '</ul></details>';
    }
    if (state !== "not_checked") {
  ...
  function showView(name) {
    ...
    if (name === "trend") trendController.init();
    updateHash();
  }
  ...
  function initGraph() {
    ...
    // Force-directed layouts look strange with very few nodes — below 20,
  ...
  var trendController = window.SdrVisualizerTrend.create({
    payload: payload,
    view: $trendView,
    log: document.getElementById("trend-log"),
